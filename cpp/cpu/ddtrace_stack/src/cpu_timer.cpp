...
render_raw_sample(EchionSampler& echion, CaptureState& state, const RawSample& raw)
{
    ...
    // Pyroscope patch: render the captured physical stack directly.
    //
    // Upstream first looks raw.python_thread_id up in echion's thread_info_map
    // and, when it hits, hands the stack to ThreadInfo::sample_cpu_timer(),
    // which re-walks asyncio tasks (or greenlets) at drain time and stitches
    // the matching logical ancestry on top of the physical frames. That whole
    // layer is Datadog's asyncio/greenlet attribution, which this vendoring
    // does not include (see VENDOR.md, "Other things dropped"), so there is
    // nothing for it to attribute to: with no Python layer populating them,
    // asyncio_loop is always 0 and greenlet_info_map is always empty, and
    // sample_cpu_timer() would fall straight through to exactly the render
    // below.
    //
    // Taking upstream's fallback branch unconditionally therefore produces the
    // same output while keeping the +500 lines of task-identity matching -- and
    // the thread_info_map lock this drain would otherwise take on every
    // sample -- out of the tree. raw.asyncio_task, raw.greenlet_id and
    // raw.coroutine_fingerprints are still captured by the (deliberately
    // byte-identical) signal handler and go unread here, in the same way
    // src/thread_span_links.cpp is still compiled but inert.
...
Engine::debug_stats() const
{
    ...
    stats.stopped_once = g_state.stopped_once.load(std::memory_order_acquire);
    stats.min_interval_ms = kMinIntervalMs;
    ...
    std::lock_guard<std::mutex> lock(g_state.registry_lock);
    stats.configuration_frozen = g_state.configuration_frozen;
