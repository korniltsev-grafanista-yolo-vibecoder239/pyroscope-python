...
struct DebugStats
{
    ...
    // Pyroscope patch: three more fields, so that everything needed to answer
    // "could start() succeed right now?" is readable without mutating anything.
    //
    // Upstream never has to ask: the CPU timer is an opt-in env var that
    // degrades quietly when it cannot run. Pyroscope raises out of configure()
    // instead, which means src/pyroscope_entry.cpp needs a pre-flight check --
    // and a pre-flight that called configure() to find out would not be one.
    //
    // configuration_frozen and stopped_once are already tracked by EngineState;
    // min_interval_ms is the kMinIntervalMs floor, which lives in an anonymous
    // namespace inside src/cpu_timer.cpp. Publishing it here beats copying the
    // number into the entry point and letting the two drift.
